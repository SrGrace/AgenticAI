...
class lpwriter():
    def __init__(self):
        ...
        self.model = watsonx_model(temperature=0)
        self.PLAN_PROMPT = ("[INST]"
                            "You are an AI/ML expert LinkedIn post writer tasked with writing a latest and high level outline of a crisp and concise LinkedIn post."
                            "Write such an outline for the user provided topic. Give the three main headers of an outline of "
                             "the post along with any relevant notes or instructions for the sections."
                             "[/INST]")
        self.WRITER_PROMPT = ("[INST]"
                              "You are an AI/ML expert LinkedIn post writing assistant tasked with writing excellent LinkedIn posts."
                              "Generate the best post possible for the user's request and the initial outline."
                              "provide relevant & actual hyperlinks from the context wherever possible."
                              "If the user provides critique, respond with a revised version of your previous attempts. "
                              "Utilize all the information below as needed: \n"
                              "------\n"
                              "{content}"
                              "[/INST]")
        self.RESEARCH_PLAN_PROMPT = ("[INST]"
                                     "You are a researcher charged with providing information that can "
                                     "be used when writing the following LinkedIn post. Generate a list of search "
                                     "queries that will gather any relevant information. Only generate 3 queries max."
                                     "[/INST]")
        self.REFLECTION_PROMPT = ("[INST]"
                                  "You are a teacher grading a LinkedIn post submission."
                                  "Generate critique and recommendations for the user's submission. "
                                  "Provide detailed recommendations, including requests for length, depth, style, etc."
                                  "[/INST]")
        self.RESEARCH_CRITIQUE_PROMPT = ("[INST]"
                                         "You are a researcher charged with providing information that can "
...
class writer_gui( ):
    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm',text_size="sm")) as demo:
            ...
            with gr.Tab("Agent"):
                with gr.Row():
                    topic_bx = gr.Textbox(label="Post Topic", value="LinkedIn post on today's ({}) top 1% news, papers, models, and repos in the AI industry.".format(datetime.today().strftime('%Y-%m-%d')))
                    gen_btn = gr.Button("Generate Post", scale=0,min_width=80, variant='primary')
                    cont_btn = gr.Button("Continue Post", scale=0,min_width=80)
                with gr.Row():
                ...
                with gr.Accordion("Manage Agent", open=False):
                    ...
                    stop_after = gr.CheckboxGroup(checks,label="Interrupt After State", value=None, scale=0, min_width=400)
                    with gr.Row():
