...
class lpwriter():
    def __init__(self):
        ...
        self.PLAN_PROMPT = ("You are an AI/ML expert twitter/x post writer tasked with writing a latest and high level outline of a crisp and concise twitter/x post."
        ...
        self.WRITER_PROMPT = ("You are an AI/ML expert twitter/x post writing assistant tasked with writing excellent twitter/x post. "
                              "Generate the best post possible for the user's request and the initial outline. use hyperlinks wherever possible."
                              "If the user provides critique, respond with a revised version of your previous attempts. "
                              "Utilize all the information below as needed: \n"
                              "------\n"
                              "{content}")
        self.RESEARCH_PLAN_PROMPT = ("You are a researcher charged with providing information that can "
                                     "be used when writing the following twitter/x post. Generate a list of search "
                                     "queries that will gather any relevant information. Only generate 3 queries max.")
        self.REFLECTION_PROMPT = ("You are a teacher grading a twitter/x post submission. "
...
class writer_gui( ):
    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm',text_size="sm")) as demo:
            ...
            with gr.Tab("Agent"):
                with gr.Row():
                    topic_bx = gr.Textbox(label="Post Topic", value="twitter/x post on today's ({}) top 1% news, papers, models, and repos in the AI industry.".format(datetime.today().strftime('%Y-%m-%d')))
